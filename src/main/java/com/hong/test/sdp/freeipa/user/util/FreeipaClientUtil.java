...
import com.hong.test.sdp.freeipa.user.comp.ServerInfoComponent;
...
@Slf4j
@Component
public class FreeipaClientUtil {
     ...
     * Freeipa客户端初始化
     */
    public void init(Integer clusterId) throws BusinessException, IOException, URISyntaxException {
        // 获取数据库存储值
        // TODO 通过 集群ID && Freeipa类型 确定一条 Freeipa 的参数，参数包括 域名、（端口）、账号、密码
        JSONObject serverInfo = serverInfoComponent.get(clusterId, FreeipaConstant.NAME);
        BASE_JSON_URL = String.format(FreeipaConstant.BASE_JSON_URL, serverInfo.getString(CommonConstant.DOMAIN));
        init(serverInfo.getString(CommonConstant.USERNAME), serverInfo.getString(CommonConstant.PASSWORD));
    }
     ...
     * @return 完整地址
     */
    private String getAddress(String preUrl) {
        JSONObject serverInfo = serverInfoComponent.get();
        String domain = serverInfo.getString(CommonConstant.DOMAIN);
    ...
    public Result<String> login(@Valid LoginParam loginParam) throws IOException, URISyntaxException {
        HttpPost httpPost = HttpUtil.createHttpPost(
                getAddress(FreeipaConstant.LOGIN_URL),
    ...
    public boolean changePassword(@Valid ChangePasswordParam changePasswordParam) throws IOException, URISyntaxException {
        HttpPost httpPost = HttpUtil.createHttpPost(
                getAddress(FreeipaConstant.CHANGE_PASSWORD_URL),
